...
public class EventNode implements Iterable<EventNode> {
	...
	public static final boolean DEBUG_ACCESS_INFO = false;
	...
	static final PerThread<Map<EventNode,String>> nodeLabelMap;
	static Map<EventNode,String> finalNodeLabelMap;

	static {
		if (DEBUG_LABELS) {
			nodeLabelMap = new PerThread<>(
					RR.maxTidOption.get(),
					HashMap::new,
					(c, e) -> {
						c.putAll(e);
...
//			if (VERBOSE_GRAPH) Assert.assertTrue(sinkNode.eventNumber >= 0);
	...
	public static Tuple2<Integer, Set<RdWrNode>> addConstraints(RdWrNode firstNode, RdWrNode secondNode, long[] windowRange, LinkedList<Edge> initialEdges, LinkedList<Edge> initialEdgesToRemove, LinkedList<Edge> additionalBackEdges, LinkedList<Edge> additionalForwardEdges, LinkedList<Edge> additionalConflictingEdges) {
		...
		int i = addConstraintsIteration.time(iteration -> {
...
//		boolean addedConstraintEdge; // constraint edge added on current iteration? May create a new path for critical sections to need LS constraints added
	 ...
	 */
	public static boolean constructReorderedTrace(RdWrNode firstNode, RdWrNode secondNode, Set<RdWrNode> coreReads, File commandDir, long[] windowRange) {
...
//					generateInputFileForGraphviz(bfsCycleEdges, true, true, true);
					...
					missingEvents.add(trPrime.removeLast());
			...
			boolean verified = forwardVerifyReorderedTrace(trPrime, firstNode, secondNode, coreReads);
			if (!verified) {
			...
			int raceDistance = countEventsInWindow(trPrime.get(0), firstNode.eventNumber, secondNode.eventNumber);
			Util.println("Race verified by " + reorderName + " reordering, race distance approximately " + raceDistance + " events");
			break;
		}
		return failed;
	}

	private static void printTrace(HashSetVector<EventNode> trPrime) {
		for (EventNode node : trPrime) {
			if (node instanceof RdWrNode) {
				RdWrNode rnode = (RdWrNode) node;
				Util.log(node + ((rnode.isRead() && rnode.hasLastWriter()) ? " lastwr: " + rnode.lastWriter() : " no lastwr"));
			} else {
				Util.log(node);
			}
		}
...
//		windowMax = secondNode.eventNumber; // everything WDC-before the second node must be totally ordered before the second node's access(es)
		...
		if (!failed) failed = constructReorderedTrace(firstNode, secondNode, coreReads, commandDir, windowRange);
	...
	static Map<ShadowID, Map<Integer, RdWrNode>> buildR (EventNode firstNode, EventNode secondNode, HashSet<EventNode> missingEvents, Set<RdWrNode> coreReads, long[] windowRange) {
	...
	static HashSetVector<EventNode> backReorderTrace(RdWrNode firstNode, RdWrNode secondNode, HashSetVector<EventNode> trPrime, int white, int reorderingAttempt, HashSet<EventNode> missingEvents, Map<ShadowID, Map<Integer/*thread id*/, RdWrNode>> thrToFirstWrPerVar, Set<RdWrNode> coreReads, long[] windowRange) {
...
//			}
			...
			if (checkConstraints(white, black, e) && checkLS(heldLocks, e) && checkLW(trPrime, thrToFirstWrPerVar, coreReads, openReadsMap, e, windowRange)) {
				trPrime.add(e);
				...
				if (e instanceof RdWrNode) {
					...
					if (access.isWrite()) {
						...
						if (coreReads.contains(access)/* || access.eventNumber < windowRange[0]*/) {
...
//			}
	...
	private static boolean checkLW(HashSetVector<EventNode> trPrime, Map<ShadowID, Map<Integer, RdWrNode>> thrToFirstWrPerVar, Set<RdWrNode> coreReads, HashMap<ShadowID, Vector<RdWrNode>> openReadsMap, EventNode event, long[] windowRange) {
		...
		if (access.isWrite()) {
			...
			for (RdWrNode read : openReads) {
				...
				if (!access.equals(read.lastWriter())) {
					if (VERBOSE_GRAPH) Assert.assertTrue(coreReads.contains(read)/* || access.eventNumber < windowRange[0]*/, "Non-CORE read in openReadsMap");
					return false;
				}
			}
		} else { // access is read
			if (! (coreReads.contains(access)/* || access.eventNumber < windowRange[0]*/)) return true; // A non-CORE read can't violate LW
    ...
    static Map<ShadowID, Map<Integer/*thread id*/, RdWrNode>> findFirstWriteOfVarsPerThread(Collection<EventNode> start, Set<RdWrNode> coreReads, long[] windowRange) {
				...
				if (current instanceof RdWrNode) {
					...
					if (currentAcc.isWrite()) {
						...
						if (currentAcc.hasLastWriter() &&
								(coreReads.contains(currentAcc) || currentAcc.eventNumber < windowRange[0])) {
...
//						collectLatex.write("wr(V"+orderedRdWr.getAccesses().firstElement().var.hashCode()+")");
	...
	public String toString() {
		return String.format("%s#%d T%d", getNodeLabel(), eventNumber, threadID);
	}
	...
	private static boolean forwardVerifyReorderedTrace(Collection<EventNode> trPrime, EventNode first, EventNode second, Set<RdWrNode> coreReads) {
		...
		for (EventNode event : trPrime) {
			...
			if (event instanceof BranchNode && !potentialLWViolations.isEmpty()) {
				...
				if (branchDepReads != null) coreTraversal.addAll(branchDepReads.values());
